...
public class ActionControllerTest {
  ...
  public void whenPostListEmployees_ThenStatus200() throws Exception {
    Mockito.when(actionRepository.saveAll(any())).thenReturn(List.of(
        VALID_ACTION_ENTITY_1_WITH_THE_SAME_NAME,
        VALID_ACTION_ENTITY_2));
    this.mockMvc.perform(post(ACTION_CONTROLLER_POST)
            .contentType(MediaType.APPLICATION_JSON)
            .content(asJsonString(List.of(ACTION_REQUEST_VALID_DTO_1, ACTION_REQUEST_VALID_DTO_2))))
        .andDo(print())
        ...
        .andExpect(jsonPath("$[0].type").value("START_TRANSACTION"))
        .andExpect(jsonPath("$[0].source.type").value("EMAIL"))
        ...
        .andExpect(jsonPath("$[1].type").value("FINISH_TRANSACTION"))
        .andExpect(jsonPath("$[1].source.type").value("PHONE"))
  ...
  public void whenPostIncorrectParameter_ThenStatus400() throws Exception {
    this.mockMvc.perform(post(ACTION_CONTROLLER_POST)
            .contentType(MediaType.APPLICATION_JSON)
            .content(asJsonString(ACTION_REQUEST_VALID_DTO_1)))
        .andDo(print())
